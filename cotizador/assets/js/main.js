$('document').ready(() => {
    ...
    function get_quote() {
        ...
        $.ajax({
            ...
            if (res.status === 200) {
                ...
                project.val(res.data.quote.project);
                noquote.val(res.data.quote.noquote);
    ...
    function add_to_quote(e) {
        ...
        // Validar el precio
        if (precio < -1000) {
            notify('Por favor ingresa un precio mayor a $5.00', 'danger');
            errors++;
        }

        // Validar la marca
        if (marca.length < 3) {
            notify('Por favor ingresa una marca válida', 'danger');
            errors++;
        }
    ...
    function edit_concept(e) {
        ...
        $.ajax({
            ...
            if (res.status === 200) {
                ...
                $('#marca', form_update_concept).val(res.data.brand);
    ...
    function save_concept(e) {
        ...
        // Validar la amrca
        if (marca.length < 2) {
            notify('Por favor ingrese una marca válida', 'danger');
            errors++;
        }
    ...
    function generate_quote(e) {
        ...
        if (proyecto.length < 2) {
            notify('Ingresa una proyecto válido, por favor', 'danger');
            errors++;
        }

        if (nocoti.length < 1) {
            notify('Ingresa una cifra válida, por favor', 'danger');
            errors++;
        }
        ...
        $.ajax({
            ...
            dataType: 'json',
            cache: false,
            data: { action, nombre, empresa, email, proyecto, nocoti },
            beforeSend: () => {
                $('body').waitMe();
